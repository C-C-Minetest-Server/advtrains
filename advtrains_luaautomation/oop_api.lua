...
function train_ref_class:split_at_index(index, cmd)
    ...
    if new_id then
        minetest.after(0.1, advtrains.atc.train_set_command, advtrains.trains[new_id], cmd, self.atc_arrow)
        return true
    end
    return false
end
...
function train_ref_class:split_at_fc(cmd, length_limit)
    ...
    if new_id then
        minetest.after(0.1, advtrains.atc.train_set_command, advtrains.trains[new_id], cmd, atc_arrow)
    end
    return fc or ""
end
